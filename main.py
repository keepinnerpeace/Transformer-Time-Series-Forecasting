...
# 定义默认参数
    ...
    frequency: int = 100,  # 打印训练信息的频率
    training_length = 48,  # 训练序列长度
    forecast_window = 24,  # 预测序列长度
    train_csv = "train_dataset.csv",  # 训练数据集文件名
    test_csv = "test_dataset.csv",  # 测试数据集文件名
    path_to_save_model = "save_model/",  # 保存模型的路径
    path_to_save_loss = "save_loss/",  # 保存损失的路径
    path_to_save_predictions = "save_predictions/",  # 保存预测结果的路径
    device = "cpu"  # 训练设备
):
